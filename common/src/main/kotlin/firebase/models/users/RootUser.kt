...
open class RootUser(val remoteRootUserRecord: RootUserRecord) :
    ...
    fun getOrdinalEntriesForProject(project: SharedProject) = userWrapper.ordinalEntries[project.projectKey.key]
        ?.mapValues {
            val projectCustomTimeIdAndKeyProvider = project.projectRecord
                .let { it as? JsonTime.ProjectCustomTimeIdAndKeyProvider }
                ?: JsonTime.ProjectCustomTimeIdAndKeyProvider.rootTask

            ProjectOrdinalManager.OrdinalEntry.fromJson(projectCustomTimeIdAndKeyProvider, it.value)
        }
        .orEmpty()
}