...
class RemoteTask<T : RemoteCustomTimeId>(
    ...
    init {
        remoteSchedules.addAll(remoteTaskRecord.remoteSingleScheduleRecords
                .values
                .map { SingleSchedule(domainFactory, RemoteSingleScheduleBridge(remoteProject.remoteProjectRecord, it)) })

        remoteSchedules.addAll(remoteTaskRecord.remoteDailyScheduleRecords
                .values
                .map { WeeklySchedule(domainFactory, RemoteDailyScheduleBridge(remoteProject.remoteProjectRecord, it)) })

        remoteSchedules.addAll(remoteTaskRecord.remoteWeeklyScheduleRecords
                .values
                .map { WeeklySchedule(domainFactory, RemoteWeeklyScheduleBridge(remoteProject.remoteProjectRecord, it)) })

        remoteSchedules.addAll(remoteTaskRecord.remoteMonthlyDayScheduleRecords
                .values
                .map { MonthlyDaySchedule(domainFactory, RemoteMonthlyDayScheduleBridge(remoteProject.remoteProjectRecord, it)) })

        remoteSchedules.addAll(remoteTaskRecord.remoteMonthlyWeekScheduleRecords
                .values
                .map { MonthlyWeekSchedule(domainFactory, RemoteMonthlyWeekScheduleBridge(remoteProject.remoteProjectRecord, it)) })
    }
        ...
        for (scheduleData in scheduleDatas) {
            ...
            when (scheduleData) {
                is CreateTaskViewModel.ScheduleData.Single -> {
                    ...
                    remoteSchedules.add(SingleSchedule(domainFactory, RemoteSingleScheduleBridge(remoteProject.remoteProjectRecord, remoteSingleScheduleRecord)))
                }
                is CreateTaskViewModel.ScheduleData.Weekly -> {
                    for (dayOfWeek in scheduleData.daysOfWeek) {
                        ...
                        remoteSchedules.add(WeeklySchedule(domainFactory, RemoteWeeklyScheduleBridge(remoteProject.remoteProjectRecord, remoteWeeklyScheduleRecord)))
                    }
                }
                is CreateTaskViewModel.ScheduleData.MonthlyDay -> {
                    ...
                    remoteSchedules.add(MonthlyDaySchedule(domainFactory, RemoteMonthlyDayScheduleBridge(remoteProject.remoteProjectRecord, remoteMonthlyDayScheduleRecord)))
                }
                is CreateTaskViewModel.ScheduleData.MonthlyWeek -> {
                    ...
                    remoteSchedules.add(MonthlyWeekSchedule(domainFactory, RemoteMonthlyWeekScheduleBridge(remoteProject.remoteProjectRecord, remoteMonthlyWeekScheduleRecord)))
                }
            }
        }
    }
