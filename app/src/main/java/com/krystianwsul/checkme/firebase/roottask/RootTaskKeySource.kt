...
class RootTaskKeySource(private val domainDisposable: CompositeDisposable) {
    ...
    fun onTaskAddedLocally(parentKey: ParentKey, taskKey: TaskKey, taskRecord: RootTaskRecord) {
        // todo task edit2 not sure if this will require rootTaskRecord; depends on RootTaskManager impl
    ...
    fun onRootTaskAddedOrUpdated(parentRootTaskKey: TaskKey.Root, childRootTaskKeys: Set<TaskKey.Root>) {
        ...
        /**
         * todo task edit2 this is going to be tricky.  When we add a task locally, we want to set up the listeners
     ...
     * longer needed.
     *
     * todo task edit2 If we end up do fine-grained updates for local edits to tasks (for child task keys), consider
