...
public class MonthlyWeekScheduleRecord extends Record {
    ...
    @NonNull
    static ArrayList<MonthlyWeekScheduleRecord> getMonthlyWeekScheduleRecords(@NonNull SQLiteDatabase sqLiteDatabase, @NonNull List<Integer> scheduleIds) {
        Assert.assertTrue(!scheduleIds.isEmpty());

        ArrayList<MonthlyWeekScheduleRecord> monthlyWeekScheduleRecords = new ArrayList<>();

        Cursor cursor = sqLiteDatabase.query(TABLE_MONTHLY_WEEK_SCHEDULES, null, COLUMN_SCHEDULE_ID + " IN (" + TextUtils.join(", ", scheduleIds) + ")", null, null, null, null);
        cursor.moveToFirst();
